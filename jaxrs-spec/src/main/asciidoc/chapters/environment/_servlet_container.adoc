...
[NOTE]
====
The use of `@Context` for injecting Servlet-defined types is deprecated.
...
Implementations MUST NOT register a CDI bean for a Servlet-defined type if a
bean satisfying that type is already available (for example, one provided by
the Jakarta Servlet or Jakarta EE Platform CDI integration), to avoid ambiguous
dependency resolution. Where no such bean is available, implementations MAY
register their own CDI producer for these types as a fallback.
====
