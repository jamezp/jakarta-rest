...
subclasses. Providers and `Application` subclasses default to application
scope (see <<cdi-injection-model>>); it is RECOMMENDED that these classes
remain application-scoped, since they are typically stateless, but
applications MAY override the default with an explicit CDI scope.
...
act as CDI qualifiers. Implementations MUST make these qualified injection points
resolvable through CDI, extracting the value from the current request context at
injection time, for any type a resource or provider declares—including
application-defined converter types and generic collections. Implementations MAY
satisfy this requirement using CDI producer methods, synthetic beans, or portable
extensions, as appropriate to the declared type. Where a producer method is used, it
MUST be `@Dependent` scoped and MAY use the CDI `InjectionPoint` API to determine the
...
NOTE: Jakarta REST parameter annotations (`@QueryParam`, `@PathParam`, etc.) MUST
only be used on injection points of a bean whose scope guarantees a new bean
instance per Jakarta REST request (the default `@RequestScoped` scope for
`@Path`-annotated classes satisfies this). Applications that violate this
constraint have undefined results, including the possibility of a stale value
being retained and reused across unrelated requests. Implementations MAY detect
and reject known-unsafe usages at deployment time.
...
* **Bean parameters**: Annotated with `@BeanParam` to inject a custom parameter
  aggregator object. The `@BeanParam` annotation is a CDI qualifier. When running
  with an active CDI container, the aggregator bean is created and injected by CDI
  like any other CDI-managed type: `@Inject` fields are injected normally, and
  fields annotated with parameter extraction annotations are resolved via the same
  qualifier mechanism used for resource method parameters (see <<cdi-producers>>).
...
@GET
@Path("search")
public List<Item> search(@BeanParam SearchParams params) {  // CDI creates and injects params
