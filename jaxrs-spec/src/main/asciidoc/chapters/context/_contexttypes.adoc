...
For `Client` instances and `SeBootstrap`-based deployments, `@Inject` for
`Configuration` and `Providers` MUST be supported when CDI is available.
Where no CDI container is present, the deprecated `@Context` annotation
remains fully supported for these types.
====
