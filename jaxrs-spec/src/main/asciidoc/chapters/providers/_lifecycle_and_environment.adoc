...
In CDI environments, a constructor that declares parameters MUST be annotated
with `@Inject` and MAY include parameters for Jakarta REST context types and
parameter extraction as described in <<cdi-injection-model>>. A public
zero-argument constructor remains valid without `@Inject`, consistent with
normal CDI bean constructor rules. In non-CDI environments, a public constructor
